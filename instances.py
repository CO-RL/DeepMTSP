...
def generate_LSCP(random, filename, n_customers, n_facilities, radius):
    """
    Generate a Location Set Covering Problem following
    ...
    with open(filename, 'w') as file:
        ...
        file.write(" obj: "+"".join([f" +1 y_{j+1}" for j in range(n_facilities)]))
        file.write("\nsubject to\n")
        ...
        for i in range(n_customers):
            cnt = cnt+1
            file.write(f" constraints{cnt}: " +"".join([f"+ {D[i,j]} y_{j+1}" for j in range(n_facilities)]) + f" >= 1\n")
...
def generate_MCLP(random, filename, n_customers, n_facilities, radius, p):
    """
    Generate a Maximal Covering Location Problem following
    ...
    with open(filename, 'w') as file:
        ...
        file.write(" obj: "+"".join([f" +1 x_{i+1}" for i in range(n_customers)]))
        file.write("\nsubject to\n")
        ...
        for i in range(n_customers):
            cnt = cnt+1
            file.write(f" constraints{cnt}: " +"".join([f"+ {D[i,j]} y_{j+1}" for j in range(n_facilities)]) + f"-1 x_{i+1} >= 0\n")
        
        cnt = cnt+1
        # facilty constraint
        file.write(f" constraints{cnt}: " +"".join([f"+ 1 y_{j+1}" for j in range(n_facilities)]) + f" = {p}\n")
        
        file.write("bounds\n")
        for i in range(n_customers):
            file.write(f" 0 <= x_{i+1} <= 1\n")
        ...
        file.write("".join([f" x_{i+1} \n" for i in range(n_customers)]))
        
        file.write("".join([f" y_{j+1} \n" for j in range(n_facilities)]))
        file.write("End")

def generate_TSP(random, filename, n_customers):
    """
    Generate a TSP file.
    ...
    with open(filename, 'w') as file:
        ...
        for i in range(1,n_customers):
            for j in range(1,n_customers):
                cnt = cnt+1
                if i == j:
                    file.write(f" c{cnt}: {n_customers} x_{i+1}_{j+1} <= {n_customers-1} \n")
                else:
                    file.write(f" c{cnt}: {n_customers} x_{i+1}_{j+1} + u_{i+1} - u_{j+1} <= {n_customers-1} \n")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        choices=['setcover', 'cauctions', 'facilities', 'indset', 'p_median', 'p_center', 'LSCP', 'MCLP', 'TSP', 'MTSP', 'minmax-mtsp'],
    )
    parser.add_argument(
        '-s', '--seed',
    ...
    if args.problem == 'setcover':
        ...
        # train instances
        n = 10000
        lp_dir = f'data/instances/p_center/train_{number_of_customers}_{number_of_facilities}_{p}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        ...
        # validation instances
        n = 2000
        lp_dir = f'data/instances/p_center/valid_{number_of_customers}_{number_of_facilities}_{p}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        number_of_customers = 100
        number_of_facilities = 10
        lp_dir = f'data/instances/p_center/transfer_{number_of_customers}_{number_of_facilities}_{p}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        number_of_customers = 100
        number_of_facilities = 10
        lp_dir = f'data/instances/p_center/test_{number_of_customers}_{number_of_facilities}_{p}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        # actually generate the instances
        for filename, ncs, nfs, p in zip(filenames, ncustomerss, nfacilitiess, ps): #zip() 函数用于将可迭代的对象作为参数，
                                                                                        #将对象中对应的元素打包成一个个元组，然后返回由这些元组组成的列表
            print(f"  generating file {filename} ...")
            generate_p_center(rng, filename, n_customers=ncs, n_facilities=nfs, p=p)
        ...
        # train instances
        n = 10000
        lp_dir = f'data/instances/LSCP/train_{number_of_customers}_{number_of_facilities}_{radius}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])   #extend() 函数用于在列表末尾一次性追加另一个序列中的多个值
        ncustomerss.extend([number_of_customers] * n)
        nfacilitiess.extend([number_of_facilities] * n)
        radiuss.extend([radius] * n)

        # validation instances
        n = 2000
        lp_dir = f'data/instances/LSCP/valid_{number_of_customers}_{number_of_facilities}_{radius}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        number_of_customers = 100
        number_of_facilities = 100
        lp_dir = f'data/instances/LSCP/transfer_{number_of_customers}_{number_of_facilities}_{radius}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        number_of_customers = 100
        number_of_facilities = 100
        lp_dir = f'data/instances/LSCP/test_{number_of_customers}_{number_of_facilities}_{radius}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        # actually generate the instances
        for filename, ncs, nfs, r in zip(filenames, ncustomerss, nfacilitiess, radiuss): #zip() 函数用于将可迭代的对象作为参数，
                                                                                        #将对象中对应的元素打包成一个个元组，然后返回由这些元组组成的列表
            print(f"  generating file {filename} ...")
            generate_LSCP(rng, filename, n_customers=ncs, n_facilities=nfs, radius=r)
        ...
        # train instances
        n = 10000
        lp_dir = f'data/instances/MCLP/train_{number_of_customers}_{number_of_facilities}_{radius}_{p}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])   #extend() 函数用于在列表末尾一次性追加另一个序列中的多个值
        ncustomerss.extend([number_of_customers] * n)
        ...
        # validation instances
        n = 2000
        lp_dir = f'data/instances/MCLP/valid_{number_of_customers}_{number_of_facilities}_{radius}_{p}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        number_of_customers = 100
        number_of_facilities = 100
        lp_dir = f'data/instances/MCLP/transfer_{number_of_customers}_{number_of_facilities}_{radius}_{p}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        number_of_customers = 100
        number_of_facilities = 100
        lp_dir = f'data/instances/MCLP/test_{number_of_customers}_{number_of_facilities}_{radius}_{p}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        # actually generate the instances
        for filename, ncs, nfs, r ,p in zip(filenames, ncustomerss, nfacilitiess, radiuss, ps): #zip() 函数用于将可迭代的对象作为参数，
                                                                                        #将对象中对应的元素打包成一个个元组，然后返回由这些元组组成的列表
            print(f"  generating file {filename} ...")
            generate_MCLP(rng, filename, n_customers=ncs, n_facilities=nfs, radius=r, p=p)
        ...
        lp_dir = f'data/instances/TSP/transfer_{number_of_customers}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        # actually generate the instances
        for filename, ncs in zip(filenames, ncustomerss): #zip() 函数用于将可迭代的对象作为参数，
                                                                                        #将对象中对应的元素打包成一个个元组，然后返回由这些元组组成的列表
            print(f"  generating file {filename} ...")
            generate_TSP(rng, filename, n_customers=ncs)
        ...
        number_of_customers = 12
        number_of_salesman = 3
        lp_dir = f'data/instances/MTSP/transfer_{number_of_customers}_{number_of_salesman}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
        ...
        number_of_salesman = 3
        lp_dir = f'data/instances/MTSP/test_{number_of_customers}_{number_of_salesman}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
