...
class GCNPolicy(BaseModel):
    ...
    def build(self, input_shapes):
        ...
        if not self.built:
            ...
            self.Q.build(emb_shape)
            self.K.build(emb_shape)
            self.V.build(emb_shape)
    ...
    def call(self, inputs, training):
        ...
        Q = self.Q(variable_features)
        K = self.K(variable_features)
        V = self.V(variable_features)

        attention = tf.matmul(tf.transpose(Q), K)/8
        attention = tf.nn.softmax(attention)
        variable_features = tf.matmul(V, attention)
        # print('1')
